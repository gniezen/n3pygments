...
class SparqlLexer(RegexLexer):
    ...
    tokens = {
        ...
        'root': [
            ...
            (r'(\s*#.*)', Comment),
            (r'\s*(SELECT\s*(?:DISTINCT|REDUCED)?)(\s*)',bygroups(Keyword,Text), 'selectVars'),
            (r'\s*((?:ASK|CONSTRUCT|DESCRIBE)\s*(?:DISTINCT|REDUCED)?\s*)((?:\?[a-zA-Z0-9_-]+\s*)+|\*)(\s*)',bygroups(Keyword,Name.Variable,Text)),
            ...
            (r'\s*(CONSTRUCT)?\s*({)',bygroups(Keyword,Punctuation),'graph'),
            (r'\s*(FROM\s*(?:NAMED)?)(\s*.*)', bygroups(Keyword,Text)),
            (r'\s*(WHERE)?\s*({)',bygroups(Keyword, Punctuation),'groupgraph'),
            (r'\s*(LIMIT|OFFSET)(\s*[+-]?[0-9]+)',bygroups(Keyword,Literal.String)),
			(r'(ORDER BY (?:ASC|DESC)\s*)(\()\s*',bygroups(Keyword,Punctuation),'bindgraph'),
            (r'\s*(})', bygroups(Punctuation)), 
        ],
        'selectVars':[
            (r'\s*(\*)\s*',bygroups(Keyword), '#pop'),
            (r'(?=\s*(FROM|WHERE|GROUP|HAVING|ORDER|LIMIT|OFFSET))', Text, '#pop'),
            (r'\s*(\()\s*', bygroups(Punctuation), 'bindgraph'),
            include('variable'),
            (r'\n', Text),
            (r'', Text, '#pop'),
        ],
        'quaddata':[
            (r'\s*({)\s*(GRAPH)(\s*<[^> ]*>\s*)', bygroups(Punctuation, Keyword, Name.Attribute), 'quads'),
            (r'\s*({)\s*',bygroups(Punctuation), 'graph'),
            (r'', Text, '#pop'),
        ],
        'quads':[
            (r'\s*({)\s*(GRAPH)(\s*<[^> ]*>\s*)', bygroups(Punctuation, Keyword, Name.Attribute), 'quads'),
            (r'\s*({)\s*', Punctuation, 'graph'),
            (r'\s*(})\s*', Punctuation, '#pop'),
        ],
        'groupgraph':[
            (r'\s*(UNION)\s*({)\s*', bygroups(Keyword, Punctuation), '#push'),
            (r'\s*({)\s*',bygroups(Punctuation), '#push'),
            include('graph'),
            include('root'),
            (r'', Text, '#pop'),
        ],
        'graph':[
            ...
            (r'\s*(BIND\s*)(\()\s*',bygroups(Keyword,Punctuation),'bindgraph'),
            (r'\s*(OPTIONAL)\s*({)',bygroups(Keyword, Punctuation), '#push'),
            (r'\s*(})\s*(\.)\s*', bygroups(Punctuation, Punctuation), '#pop'),
            (r'\s*(})', bygroups(Punctuation), '#pop'),
            (r'\s*(\.)\s*', bygroups(Punctuation), '#pop'),
        ],
        'triple' : [
            (r'(?=\s*})', Text, '#pop'),                    
            (r'\s*(\.)\s*', bygroups(Punctuation), '#pop'),
        ...
        'object': [
            ...
            (r'\s*[a-zA-Z0-9\-_\:]\s*', Name.Attribute),
            (r'\s*(\()', bygroups(Punctuation), 'objList'),
            (r',', Text),
            (r'\s*;\s*', Punctuation, '#pop'),
            (r'(?=\])', Text, '#pop'),            
            (r'\s*(?=\.)', Text, '#pop'),
        ...
        'filterConstraint':[
            ...
            (r'\s*(\()\s*', bygroups(Punctuation), 'filterExp'),
            (r'\s*(\.)\s*', bygroups(Punctuation), '#pop'),
        ],
        #filterBuiltin is intended to be included, not pushed
        'filterBuiltin':[
            include('aggregate'),
            (r'(str|lang|langmates|datatype|bound|iri|uri|bnode)\s*(\()', bygroups(Name.Builtin, Punctuation), 'objList'),
            (r'(abs|ceil|floor|round)\s*(\()', bygroups(Name.Builtin, Punctuation), 'objList'),
            (r'(strlen|ucase|lcase|encode_for_uri|contains|strstarts|strends|strbefore|strafter)\s*(\()', bygroups(Name.Builtin, Punctuation), 'objList'),
            (r'(year|month|day|hours|minutes|seconds|timezone|tz)\s*(\()', bygroups(Name.Builtin, Punctuation), 'objList'),
            (r'(md5|sha1|sha256|sha384|sha512)\s*(\()', bygroups(Name.Builtin, Punctuation), 'objList'),
            (r'(if|strlang|strdt)\s*(\()', bygroups(Name.Builtin, Punctuation), 'objList'),
            (r'(sameterm|isIRI|isURI|isBlank|isLiteral|isNumeric)\s*(\()', bygroups(Name.Builtin, Punctuation), 'objList'),
            (r'(regex)\s*(\()', bygroups(Name.Builtin, Punctuation), 'objList'),
        ],
        ...
        'bindgraph':[
            (r'\s*(\()\s*', bygroups(Punctuation), '#push'),
            (r'\s*AS\s*', Keyword),
            (r'\s*(IRI\s*)(\()\s*',bygroups(Keyword,Punctuation),'iri'),
            (r'\s*(\))\s*', bygroups(Punctuation), '#pop'),
            include('filterExp'),
            include('variable'),
            include('object'),
